...
`GET /listings?q=&category=&min_price=&max_price=&limit=&offset=`

- `q` — полнотекстовый поиск по заголовку и описанию (Postgres FTS,
  generated-колонка + GIN-индекс, конфигурация `simple` — работает для
  русского и английского вперемешку). Синтаксис — как в веб-поиске
  (`websearch_to_tsquery`), максимум 100 символов. Комбинируется с
